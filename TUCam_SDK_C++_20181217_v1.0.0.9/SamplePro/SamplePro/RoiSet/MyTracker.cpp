...
void CMyTracker::Draw(CDC* pDC) const
{
	...
	if ((m_nStyle & (dottedLine|solidLine)) != 0)
	{
        ...
        CString sz;
        int nwid = (int)(rect.Width() * m_fScale * 1.0f);
        int nhei = (int)(rect.Height() * m_fScale * 1.0f);
        ////sz.Format(_T("%d, %d"), min((nwid >> 2) << 2, 2048), min((nhei >> 2) << 2, 2048));
		sz.Format(_T("%d, %d"), min((nwid >> 3) << 3, 2048), min((nhei >> 3) << 3, 2048));
        CRect rc = CRect(rect.left, rect.top - 20, rect.left + 100, rect.top);
        pDC->DrawText(sz, -1, &rc, DT_LEFT);
...
void CMyTracker::SetScale(float fScale /*= 1.0f*/)
{
    m_fScale = fScale;
}
...
{    
	...
	for (;;)
	{
		...
		case WM_MOUSEMOVE:
			...
			// handle move case
			if (nHandle == hitMiddle)//jorgo 6.10 add
			{	
				if (*px < m_PreviewRt.left)	m_rect.left = m_PreviewRt.left;
				if (*py < m_PreviewRt.top)	m_rect.top = m_PreviewRt.top;
				if (*px > m_PreviewRt.right-nWidth) m_rect.left = m_PreviewRt.right-nWidth;
				if (*py > m_PreviewRt.bottom-nHeight)	m_rect.top = m_PreviewRt.bottom-nHeight;
				
				m_rect.right = m_rect.left + nWidth;
				m_rect.bottom = m_rect.top + nHeight;
			}
			if (nHandle == hitTopLeft || nHandle == hitTop || nHandle == hitTopRight)
			{
				if (*py < m_PreviewRt.top) m_rect.top = m_PreviewRt.top;
				if (*py > m_PreviewRt.bottom) m_rect.top = m_PreviewRt.bottom;	
			}
			
			if (nHandle == hitTopLeft || nHandle == hitLeft || nHandle == hitBottomLeft)
			{
				if (*px < m_PreviewRt.left) m_rect.left = m_PreviewRt.left;
				if (*px > m_PreviewRt.right) m_rect.left = m_PreviewRt.right;			
			}
			
			if (nHandle == hitBottomLeft || nHandle == hitBottom || nHandle == hitBottomRight)
			{
				if (*py > m_PreviewRt.bottom)	m_rect.bottom = m_PreviewRt.bottom;
				if (*py < m_PreviewRt.top)	m_rect.bottom = m_PreviewRt.top;
			}
			
			if (nHandle == hitTopRight || nHandle == hitRight || nHandle == hitBottomRight)
			{
				if (*px > m_PreviewRt.right)	m_rect.right = m_PreviewRt.right;
				if (*px < m_PreviewRt.left)	m_rect.right = m_PreviewRt.left;
 			}
...
void CMyTracker::CreatePen()
{
	    ...
	    //if (_afxBlackDottedPen == NULL)
		{
			...
			_afxBlackDottedPen = ::CreatePen(PS_DOT, 2, m_rectColor);
			if (_afxBlackDottedPen == NULL)
			{
				AfxUnlockGlobals(CRIT_RECTTRACKER);
				AfxThrowResourceException();
		...
		{
			// create black dotted pen
			_afxBlackSolidPen = ::CreatePen(PS_SOLID, 2, m_rectColor);
			if (_afxBlackSolidPen == NULL)
			{
				AfxUnlockGlobals(CRIT_RECTTRACKER);
				AfxThrowResourceException();
...
void CMyTracker::SetPreviewRect(CRect &rt)
{
	m_PreviewRt = rt;
}
